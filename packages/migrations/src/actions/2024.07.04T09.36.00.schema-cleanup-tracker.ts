...
export default {
  name: '2024.07.04T09.36.00.schema-cleanup-tracker.ts',
  async run({ connection }) {
    ...
    // Fetch targets
    const targetIds = await connection.manyFirst<string>(sql`
    ...
    for await (const targetId of targetIds) {
      console.log(`Processing target (${i++}/${targetIds.length}) - ${targetId}`);
      ...
      if (!latestSchema) {
        console.log('[SKIPPING] No latest composable schema found for target %s', targetId);
        continue;
      }
      ...
      await processVersion(1, connection, targetCoordinates, targetId, {
        schema,
