...
PARTITION BY tuple()
PRIMARY KEY (target, hash)
ORDER BY (target, hash, client_name, client_version, timestamp, expires_at)
...
CREATE MATERIALIZED VIEW IF NOT EXISTS default.operations_minutely
(
  target LowCardinality(String) CODEC(ZSTD(1)),
  timestamp DateTime('UTC') CODEC(DoubleDelta, LZ4),
  expires_at DateTime('UTC') CODEC(DoubleDelta, LZ4),
...
CREATE MATERIALIZED VIEW IF NOT EXISTS default.operation_collection_body
(
  target LowCardinality(String) CODEC(ZSTD(1)),
  hash String CODEC(ZSTD(1)), 
  body String CODEC(ZSTD(1)),
...
-- It deduplicates the rows and stores them only once.
-- Should significantly reduce the size of the original table.
CREATE MATERIALIZED VIEW IF NOT EXISTS default.operation_collection_details
(
  target LowCardinality(String) CODEC(ZSTD(1)),
  name String CODEC(ZSTD(1)),
  hash String CODEC(ZSTD(1)), 
