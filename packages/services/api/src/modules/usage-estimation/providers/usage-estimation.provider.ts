import type {
  UsageEstimatorApi,
  UsageEstimatorQueryInput,
} from '@hive/usage-estimator';
...
export class UsageEstimationProvider {
  ...
  async estimateOperations(
    ...
    if (!this.usageEstimator) {
      this.logger.warn(
        'Usage estimator is not available due to missing configuration'
      );

      return null;
    }

    const result = await this.usageEstimator.query(
      'estimateOperationsForTarget',
      input
    );
  ...
  async estimateSchemaPushes(
    input: UsageEstimatorQueryInput<'estimateSchemaPushesForTarget'>
  ): Promise<null | number> {
