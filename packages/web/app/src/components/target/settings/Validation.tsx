...
const Check = {
  Label: styled.label(({ selected, disabled }: { selected: boolean; disabled?: boolean }) => [
...
const Checkbox: React.FC<{
  ...
  return (
    <Check.Label selected={selected} disabled={disabled}>
      <Check.Input type="checkbox" name={name} value={value} onBlur={onBlur} onChange={onChange} disabled={disabled} />
...
export const ValidationSettings: React.FC<{
  ...
  const [, updateValidation] = useMutation(UpdateTargetValidationSettingsDocument);
  ...
  return (
    <Card.Root>
      ...
      <Card.Content>
        <form onSubmit={formik.handleSubmit} style={{ opacity: enabled ? 1 : 0.2 }}>
          ...
          <Description tw="mt-6">Example settings: Removal of a field is considered breaking if</Description>
          <Description tw="mt-3">
            <Label>0%</Label> - the field was used at least once in past 30 days
          </Description>
          <Description tw="mt-2">
            <Label>10%</Label> - the field was requested by more than 10% of all GraphQL operations in recent 30 days
          </Description>
          <div tw="flex flex-row mt-6 space-x-4 items-center">
            <Button colorScheme="primary" disabled={disabled || !formik.isValid} onClick={formik.handleSubmit as any}>
              Save
            </Button>
          </div>
        </form>
