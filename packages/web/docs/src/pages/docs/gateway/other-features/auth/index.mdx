...
- [JSON Web Tokens (JWT)](/docs/gateway/other-features/auth/jwt): Hive Gateway provides a plugin to
  easily integrate JWT token verification into your API.
- [Generic Auth](/docs/gateway/other-features/auth/generic-auth): In some cases using a third party
  auth provider is not possible. But now worries, the generic auth plugin has you covered!
...
- [Operation Field Permissions](/docs/gateway/other-features/auth/operation-field-permissions): You
  can automatically reject GraphQL operations that include specific field selections within the
