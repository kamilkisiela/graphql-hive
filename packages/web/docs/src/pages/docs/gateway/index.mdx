...
- [JSON Web Tokens (JWT)](/docs/gateway/other-features/authorization-authentication) authentication
  between client and gateway also between gateway and subgraph
- [Authorization](/docs/gateway/other-features/authorization-authentication) on specific fields and
  types using Federation Auth directives like `@authenticated` and `@skipAuth`
- [Role-based Access Control (RBAC)](/docs/gateway/other-features/authorization-authentication)
  either programmatically or with directives `@requiresScope` and `@policy`
...
- [Prometheus and OpenTelemetry integration](/docs/gateway/other-features/monitoring-tracing) with
  fully customizable spans and attributes
- [Persisted Documents](/docs/gateway/other-features/performance/persisted-operations) backed by
  either Hive Registry or a selfhosting storage
