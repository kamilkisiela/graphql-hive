---
description:
  Monitoring and tracing are essential for debugging and understanding the performance of your Hive
  Gateway.
---
...
> The `payload` object is the same as the one passed to the
> [`onSubgraphHook` hook](/docs/gateway/features/custom-plugins#onsubgraphexecute).
...
> The `payload` object is the same as the one passed to the
> [`onFetch` hook](/docs/gateway/features/custom-plugins#onfetch).
...
[Plugin Lifecycle page](/docs/gateway/features/custom-plugins#plugin-lifecycle). Each plugin hook
has a corresponding metric which tracks timings as
