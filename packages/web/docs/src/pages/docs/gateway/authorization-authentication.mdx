...
# Authorisation and Authentication (JWT)

Hive Gateway supports Authentication and Authorisation using JSON Web Tokens (JWT).
...
use [HMAC Signature](/docs/gateway/features/security/hmac-signature) between your Hive Gateway and
the subgraphs. This will ensure that the requests to the subgraphs are trusted and signed by the
...
[Operation Field Permissions](/docs/gateway/features/security/auth/operation-field-permissions), you
can use the `jwt` property of the context to access the decoded JWT token, and decide what
permissions to allow to the user based on identity or token claims:
...
[Generic Auth plugin](/docs/gateway/features/auth/generic-auth) to have a granular protection using
with or without JWT.
...
[Learn more about granular protection of Generic Auth plugin](/docs/gateway/features/auth/generic-auth)
