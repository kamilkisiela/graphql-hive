...
The [Hive CLI](/api-reference/cli) can be installed on any environment, including CI/CD
environments.

If you are using a JavaScript/NodeJS project, you should install the
[Hive CLI](/api-reference/cli#nodejs) under `devDependencies` of your project, and use it directly
with your preferred package manager (for example: `yarn hive ...` or `pnpm hive ...`).

If you are using a different runtime environment for your project, you should install the
[Hive CLI binary](/api-reference/cli#binary) and use it directly as a binary (`hive ...`).
...
1. For your actual runtime (`staging` and `production` targets), we recommend using a
   [Hive Client](/api-reference/client) library that runs along with your GraphQL gateway, and
   publishes the GraphQL schema, along with [Usage Reporting](/features/usage-reporting) feature
   enabled.
2. For your development process (`development` target), we recommend using CI pipelines to publish
   the schema to Hive using [Hive CLI](/api-reference/cli):
   ...
   - The `development` target may also use the
     [Cross-target checks for Conditional Breaking Changes](/management/targets#cross-target-checks)
