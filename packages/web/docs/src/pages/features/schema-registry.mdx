...
<Cards>
  <Card arrow title="Single Schema" href="/get-started/single-project" />
  <Card arrow title="Schema-Stitching" href="/self-hosting/schema-stitching-project" />
  <Card arrow title="Apollo Federation" href="/self-hosting/apollo-federation" />
</Cards>

## Actions on schemas

This section covers the actions that can be performed on GraphQL schemas in Hive. Action of schemas
can be performed using the [Hive CLI](/api-reference/cli), the
[Hive GraphQL API](/api-reference/graphql-api), or through the [Hive Client](/api-reference/client).
...
- **Hive Metadata**: Hive metadata is an optional JSON object you can attach to every published
  ...
  [Publishing Hive metadata](/api-reference/cli#hive-metadata)).
...
5. **Schema Changes** - A schema `diff` is calculated and compared against the previous schema
   version. If [Conditional Breaking Changes](/management/targets#conditional-breaking-changes) is
   activated, an additional phase of check against collected operations is performed.

For additional reading:

- [Publishing a schema using Hive CLI](/api-reference/cli#publish-a-schema)
- [`schema:publish` command reference](/api-reference/cli#hive-schemapublish-file)
- [Publishing a schema using Hive Client](/api-reference/client)
...
4. **Schema Changes** - A schema `diff` is calculated and compared against the previous schema
   version. If [Conditional Breaking Changes](/management/targets#conditional-breaking-changes) is
   activated, an additional phase of check against collected operations is performed. If a non-safe
   change has been introduced in the schema check, it will be rejected by Hive.

For additional reading:

- [Checking a schema using Hive CLI](/api-reference/cli#check-a-schema)
- [`schema:check` command reference](/api-reference/cli#hive-schemacheck-file)
...
- [Deleting a service using Hive CLI](/api-reference/cli#delete-a-schema)
- [`schema:delete` command reference](/api-reference/cli#hive-schemadelete-service)
...
- **Associated Git Commit**: the Git commit associated with the change, when
  [GitHub Integration](/management/organizations#github) is enabled for your organization, and a
  [GitHub repository is linked to the Hive project](/management/projects#github-repository).
...
With [Usage Reporting](/features/usage-reporting) feature enabled, you'll be able to see an overview
of the schema usage and coverage (for types, fields and input types), based on the GraphQL
...
<Callout type="info">
  ...
  plan](/management/organizations#subscription-and-billing), and depends on the data you already
  sent before to Hive.
</Callout>
