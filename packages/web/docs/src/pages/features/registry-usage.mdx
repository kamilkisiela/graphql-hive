...
- Raw SDL `<endpoint>/sdl`
  ...
  If you wish to get the raw SDL, without a wrapping JSON object, you can specify `/sdl` as suffix
  to your request.

- Introspection JSON `<endpoint>/introspection`

  If you wish to get the introspection JSON for your GraphQL schema, without a wrapping JSON object,
  you can specify `/introspection` as suffix to your request. This could be useful for tools that
  need to get the introspection as-is, like GraphQL Codegen, or GraphQL-Config.
