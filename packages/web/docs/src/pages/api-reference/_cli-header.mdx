...
You can perform [schema-registry actions](/features/schema-registry#actions-on-schemas) on your Hive
...
Start by setting your Hive token in [`hive.json`](/api-reference/cli#config-file-hivejson) file, or
set it as `HIVE_TOKEN` environment variable.

Further reading:

- [Publishing a schema to the Schema Registry](/features/schema-registry#publish-a-schema)
...
- [`schema:publish` API Reference](/api-reference/cli#hive-schemapublish-file)
...
- [`schema:publish` API Reference](/api-reference/cli#hive-schemapublish-file)
- [Apollo Router integration](/integrations/apollo-router)
- [Apollo-Server integration](/integrations/apollo-server)
...
- [Fetching Hive Metadata from the CDN](/features/high-availability-cdn#hive-metadata)
...
- [`schema:check` API Reference](/api-reference/cli#check-a-schema)
- [Publishing a schema to the Schema Registry](/features/schema-registry#publish-a-schema)
- [Conditional Breaking Changes](/management/targets#conditional-breaking-changes)
...
If [GitHub Integration](/management/organizations#github) is enabled for your organization, and a
[GitHub repository is linked to the Hive project](/management/projects#github-repository) is active,
you may specify an additional `--github` flag to report the results back to GitHub as Check Suite
(for `schema:check` and `schema:publish` commands):
