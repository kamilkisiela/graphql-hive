...
Users with wide screens can now benefit from the fullscreen mode. The button is located in place of the
GraphiQL logo.
...
Laboratory now supports multiple tabs, you can open tabs with different queries and switch
between them as in regular GraphiQL. There is no longer a requirement to mandatory save the query in
one of the collections, you can have a draft query which will be stored in local storage.

<Video src={newTabs} alt="Tabs demo" />

The difference Hive's tabs from regular GraphiQL tabs is the better look, I was inspired by the UX and UI of
Google Chrome tabs, and we loved the new look so much, so we decided
...
While working on improvements for the laboratory, I also cut the next major v4 alpha release of GraphiQL,
which includes a new tabs look and migration from Webpack to Vite.
