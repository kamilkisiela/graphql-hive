...
<Callout>
  **GitHub.** For receiving the `GITHUB_APP_ID` and `GITHUB_APP_PRIVATE_KEY` values follow the [Creating a GitHub
  App](https://docs.github.com/en/developers/apps/building-github-apps/creating-a-github-app) documentation.
</Callout>

<Callout>
  **Slack.** You need to create a new Slack app for receiving the `SLACK_CLIENT_ID` and `SLACK_CLIENT_SECRET` values.
  [You can create a Slack app here.](https://api.slack.com/apps/)
</Callout>
...
<Callout>
  **Docker Tag.** Docker images are built and published for each single commit within the GraphQL Hive repository and
  ...
  versions. Right now we do not have a stable release strategy hey. However, every commit on the `main` branch can be
...
export GITHUB_APP_NAME="graphql-hive"
export GITHUB_APP_ID="66666"
export GITHUB_APP_PRIVATE_KEY=$(openssl rand -hex 16)
export SLACK_CLIENT_ID=$(openssl rand -hex 16)
export SLACK_CLIENT_SECRET=$(openssl rand -hex 16)
```
