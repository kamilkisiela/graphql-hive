...
<Callout>
  We wrapped `execute` function with Hive and used it as a custom execute
  function in `express-graphql`. This is the easiest and closest way to control
  GraphQL execution.
</Callout>
...
<Callout>
  The `clientInfo` method receives a context object as the first and only
  argument and expects to get either `null`, `undefined` or `{(name, version)}`
  object.
</Callout>
