const getUser = () =>
  ({ email: `${crypto.randomUUID()}@local.host`, password: 'Loc@l.h0st', firstName: 'Local', lastName: 'Host' }) as const;
...
describe('oidc', () => {
  it('oidc login for organization', () => {
    ...
    cy.createOIDCIntegration('Bubatzbieber')
    .then(({ loginUrl }) => {
        cy.visit('/logout');
  ...
  it('oidc login with organization slug', () => {
    ...
    cy.createOIDCIntegration('Bubatzbieber')
    .then(({ loginUrl, organizationSlug }) => {
      cy.visit('/logout');
